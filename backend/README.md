...
- **Frontend geliştiricisiysen:** Önce [Hızlı başlangıç](#hızlı-başlangıç) ile ortamı ayağa kaldır, seed + simulator ile sahte veri akıt, sonra [REST + WebSocket sözleşmesi](#rest--websocket-sözleşmesi) bölümüne bak. Kesin alan referansı için servis ayaktayken `http://localhost:8000/docs` (Swagger). **Önemli:** CORS ayarlı değil, ayrı origin'den direkt çağrı yaparsan preflight'a takılırsın — bkz. [Sık takılınan yerler](#sık-takılınan-yerler).
...
- **CORS ayarlı değil.** `app/` içinde hiçbir CORS middleware yok. Tarayıcı frontend'i farklı origin/port'tan direkt çağırırsa preflight'a takılır. Aynı origin'den veya bir proxy arkasından çağır.
...
- **CORS bilinçli olarak açılmadı.** Aynı origin / proxy varsayımı geçerli.
